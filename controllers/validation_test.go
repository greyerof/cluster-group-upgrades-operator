...
const policyWithOnlyVersionAndVariableUpstream = `---
kind: Policy
spec:
  policy-templates:
  - objectDefinition:
...
func TestClusterGroupUpgradeReconciler_extractOpenshiftImagePlatformFromPolicies(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.server != nil {
				url := tt.server.URL
				policyWithOnlyVersion := fmt.Sprintf(policyWithOnlyVersionAndVariableUpstream, url)
				tt.args.policies = []*unstructured.Unstructured{mustConvertYamlStrToUnstructured(policyWithOnlyVersion)}
			}

			got, err := r.extractOCPImageFromPolicies(tt.args.policies)
...
const policyWithTemplatizedFieldsInClusterVersion = `---
kind: Policy
spec:
  policy-templates:
  - objectDefinition:
...
func TestClusterGroupUpgradeReconciler_validateOpenshiftUpgradeVersion(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name:    "with both Image and Version",
			fields:  commonFields,
			args:    args{cgu: &ranv1alpha1.ClusterGroupUpgrade{}, policies: []*unstructured.Unstructured{mustConvertYamlStrToUnstructured(policy)}},
			wantErr: assert.NoError,
		},
		{
			name:    "with only Image",
			fields:  commonFields,
			args:    args{cgu: &ranv1alpha1.ClusterGroupUpgrade{}, policies: []*unstructured.Unstructured{mustConvertYamlStrToUnstructured(policyWithOnlyImage)}},
			wantErr: assert.NoError,
		},
		{
			name:    "with only Image and version is an empty string",
			fields:  commonFields,
			args:    args{cgu: &ranv1alpha1.ClusterGroupUpgrade{}, policies: []*unstructured.Unstructured{mustConvertYamlStrToUnstructured(policyWithOnlyImageAndVersionIsEmptyString)}},
			wantErr: assert.NoError,
		},
		{
			name:    "with only Version",
		...
		},
		{
			name:    "with both Image and Version and conflicting version with in multiple clusterversiongroup",
			fields:  commonFields,
			args:    args{cgu: &ranv1alpha1.ClusterGroupUpgrade{}, policies: []*unstructured.Unstructured{mustConvertYamlStrToUnstructured(policyWithTwoClusterVersion)}},
			wantErr: assert.Error,
		},
		{
			name:    "return no error when ClusterVersion is not found",
			fields:  commonFields,
			args:    args{cgu: &ranv1alpha1.ClusterGroupUpgrade{}, policies: []*unstructured.Unstructured{mustConvertYamlStrToUnstructured(policyWithoutClusterVersion)}},
			wantErr: assert.NoError,
		},
		{
			name:   "return no error when templatized ClusterVersion is found without precaching",
			fields: commonFields,
			args: args{
				cgu:      &ranv1alpha1.ClusterGroupUpgrade{},
				policies: []*unstructured.Unstructured{mustConvertYamlStrToUnstructured(policyWithTemplatizedFieldsInClusterVersion)},
			},
			wantErr: assert.NoError,
		},
		{
			name:   "return error when templatized ClusterVersion is found with precaching",
			fields: commonFields,
			args: args{
				cgu:      &ranv1alpha1.ClusterGroupUpgrade{Spec: ranv1alpha1.ClusterGroupUpgradeSpec{PreCaching: true}},
				policies: []*unstructured.Unstructured{mustConvertYamlStrToUnstructured(policyWithTemplatizedFieldsInClusterVersion)},
			},
			wantErr: assert.Error,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := r.validateOpenshiftUpgradeVersion(tt.args.cgu, tt.args.policies)
			if !tt.wantErr(t, err, fmt.Sprintf("extractOpenshiftImagePlatformFromPolicies(%v)", tt.args.policies)) {
				return
			}
		})
